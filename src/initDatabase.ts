...
export const initDatabase = async () => {
    try {
        await checkData(FirestoreCollection.USERS, path.join(__dirname, 'users.json'));
        await checkData(FirestoreCollection.ASSIGNMENTS, path.join(__dirname, 'assignments.json'));
        await checkData(FirestoreCollection.COURSES, path.join(__dirname, 'courses.json'));
    } catch (error) {
